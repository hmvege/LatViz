...
def create_animation(
    ...
    animation_path = os.path.join(
        animation_folder,
        "%s_%s_t%d.%s" % (observable.lower(), method, time_point, anim_type),
    )
...
def load_folder_data(folder, N, NT, euclidean_time=None, flow_time=None):
    ...
    assert isinstance(euclidean_time, type(None)) ^ isinstance(
        flow_time, type(None)
    ...
    if not isinstance(flow_time, type(None)):
        ...
        if len(_tmp_folder_files) == 0:
            raise IOError(
                "No flow data with flow time {0:} found among"
                " following files: \n{1}".format(
    ...
    # Loading data
    for _f in tqdm(
        folder_files, desc="Reading in data from {}".format(folder)
    ):

        data.append(
            load_field_from_file(_f, N, NT, euclidean_time=euclidean_time)
        )
...
def main():
    ...
    # Required arguments
    parser.add_argument(
        "folder", type=str, help="Folder containing configurations."
    )
    ...
    parser.add_argument(
        ...
        default="observable",
        type=str,
        help="Name of the observable name.",
    )
    parser.add_argument(
        "-at",
        "--anim_type",
    ...
    parser.add_argument(
        "--title", default=None, type=str, help="Title of figure."
    )
    parser.add_argument(
        "-fz",
        "--figsize",
    ...
    parser.add_argument(
        "-fr", "--frame_rate", default=10, type=int, help="Frame rate."
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=float,
        help=("If provided, will correct input " "values with factor."),
    )
    ...
    assert (
        (len(args.figsize) == 2)
        and (type(args.figsize[0]) == int)
        and (type(args.figsize[1]) == int)
    ), f"incorrect figsize: {args.figsize}"

    if (args.vmin is not None) and (args.vmax is not None):
        assert args.vmin < args.vmax, (
            f"vmin is less than vmax: {args.vmin} < {args.vmax}"
        )
